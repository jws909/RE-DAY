...
<body
	class="bg-slate-100 text-slate-900 min-h-screen pb-16 font-sans antialiased">
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
		...
		<c:if test="${empty review}">
			<div class="max-w-3xl mx-auto px-4 py-16 text-center space-y-4">
				<div
					class="p-8 border-2 border-dashed border-slate-300 rounded-2xl bg-white space-y-3">
					<div
						class="w-12 h-12 rounded-full bg-slate-100 text-slate-400 mx-auto flex items-center justify-center text-xl">
		...
		<c:if test="${not empty review}">
			...
			<!-- --------------------------------------------------------------------- -->
			<article
				class="border-2 border-dashed border-slate-300 rounded-2xl bg-white p-6 sm:p-8 space-y-6 shadow-sm">
				...
				<div
					...
					<div class="flex items-center gap-3">
						...
						<c:choose>
							<c:when
								test="${not empty user.profileImg or not empty user.profile_img or not empty profileImg}">
								<img
									src="${not empty user.profileImg ? user.profileImg : (not empty user.profile_img ? user.profile_img : profileImg)}"
									alt="프로필 이미지"
									class="w-12 h-12 rounded-full object-cover border border-slate-300 shadow-sm" />
							</c:when>
							<c:otherwise>
								<div
									class="w-12 h-12 rounded-full bg-slate-900 text-white font-mono font-bold flex items-center justify-center text-base shadow uppercase">
									<c:choose>
						...
						<div>
							...
							<div class="flex items-center gap-2 flex-wrap">
								...
								<c:choose>
									...
									<c:when test="${rawLevel eq 'LV4' or rawLevel eq '4'}">
										<span id="userLevelBadge" data-level="${rawLevel}"
											class="text-xs font-mono px-2 py-0.5 rounded border border-blue-300 bg-blue-50 text-blue-700 font-semibold">
											Lv.4 프로 기록러 </span>
									</c:when>
									<c:when test="${rawLevel eq 'LV3' or rawLevel eq '3'}">
										<span id="userLevelBadge" data-level="${rawLevel}"
											class="text-xs font-mono px-2 py-0.5 rounded border border-emerald-300 bg-emerald-50 text-emerald-700 font-semibold">
											Lv.3 데일리 아카이버 </span>
									</c:when>
									<c:when test="${rawLevel eq 'LV2' or rawLevel eq '2'}">
										<span id="userLevelBadge" data-level="${rawLevel}"
											class="text-xs font-mono px-2 py-0.5 rounded border border-slate-200 bg-slate-100 text-slate-600 font-medium">
											Lv.2 루키 아카이버 </span>
									</c:when>
									<c:otherwise>
										<span id="userLevelBadge" data-level="${rawLevel}"
								...
								<c:set var="rawStreak"
									value="${not empty user.streakCount ? user.streakCount : (not empty user.streak_count ? user.streak_count : (not empty streakCount ? streakCount : 0))}" />
								<c:choose>
									<c:when test="${rawStreak >= 30}">
										<span id="userStreakBadge" data-streak="${rawStreak}"
											class="text-[11px] font-bold text-amber-800 bg-amber-50 px-2 py-0.5 rounded border border-amber-300 font-mono inline-flex items-center gap-1 shadow-2xs streak-badge">
											<i class="fa-solid fa-trophy text-amber-500 text-xs"></i> <span
											id="userStreakText">${rawStreak}일 연속 챔피언</span>
										</span>
									</c:when>
									<c:when test="${rawStreak >= 14}">
										<span id="userStreakBadge" data-streak="${rawStreak}"
											class="text-[11px] font-semibold text-orange-600 bg-orange-50 px-2 py-0.5 rounded border border-orange-200 font-mono inline-flex items-center gap-1 shadow-2xs streak-badge">
											<i class="fa-solid fa-fire text-orange-500 text-xs"></i> <span
											id="userStreakText">${rawStreak}일 연속 마스터</span>
										</span>
									</c:when>
									<c:when test="${rawStreak >= 7}">
										<span id="userStreakBadge" data-streak="${rawStreak}"
											class="text-[11px] font-semibold text-orange-600 bg-orange-50 px-2 py-0.5 rounded border border-orange-200 font-mono inline-flex items-center gap-1 shadow-2xs streak-badge">
											<i class="fa-solid fa-fire text-orange-500 text-xs"></i> <span
											id="userStreakText">${rawStreak}일 연속 챌린저</span>
										</span>
									</c:when>
									<c:when test="${rawStreak > 0}">
										<span id="userStreakBadge" data-streak="${rawStreak}"
											class="text-[11px] font-medium text-orange-600 bg-orange-50 px-2 py-0.5 rounded border border-orange-200 font-mono inline-flex items-center gap-1 shadow-2xs streak-badge">
											<i class="fa-solid fa-fire text-orange-500 text-xs"></i> <span
											id="userStreakText">${rawStreak}일 연속 기록</span>
										</span>
									</c:when>
									<c:otherwise>
								...
								<c:choose>
									<c:when
										test="${empty review.isPublic or review.isPublic eq 'Y'}">
										<span
											class="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 font-mono">
											전체공개 </span>
									</c:when>
									<c:otherwise>
										<span
											class="text-[10px] font-semibold text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 font-mono">
											비공개 </span>
									</c:otherwise>
								</c:choose>
							</div>
							...
							<div
								...
								<i class="fa-regular fa-calendar-days text-slate-400"></i> <span
									class="font-mono font-medium">${review.reviewDate}</span>
								<c:set var="dw"
					...
					<div
						class="flex flex-col sm:items-end bg-amber-50/80 p-3.5 rounded-xl border border-amber-200">
						<span
							class="text-[11px] font-bold text-amber-800 uppercase tracking-wider mb-1">
							오늘 하루 종합 평점 </span>
						<!-- 별점 5개 시각화 + 텍스트 점수 -->
						<div class="inline-flex items-center gap-1.5 select-none">
								...
								<c:forEach var="i" begin="1" end="5">
									<c:choose>
										...
										<c:when test="${review.totalRating >= (i - 0.5)}">
											<i
												class="fa-solid fa-star-half-stroke text-amber-400 text-lg"></i>
										</c:when>
										<c:otherwise>
							...
							<span class="text-slate-700 ml-1 font-mono text-lg font-bold">
								${review.totalRating} <span
								class="text-slate-400 font-normal text-sm">/ 5.0</span>
							</span>
						</div>
					</div>
				</div>
					...
					<div class="flex flex-wrap gap-2">
						<c:forEach items="${fn:split(review.moodTags, ',')}" var="tag">
							<c:if test="${not empty fn:trim(tag)}">
								<span
									class="inline-flex items-center gap-1 bg-slate-100 text-slate-700 border border-slate-300 rounded font-mono px-2 py-1 text-xs">
									<i class="fa-solid fa-tag text-[10px] text-slate-400"></i> <span><c:out
											value="${fn:startsWith(fn:trim(tag), '#') ? fn:trim(tag) : '#' += fn:trim(tag)}" /></span>
								</span>
							</c:if>
						</c:forEach>
					</div>
				...
				<div class="space-y-2">
					<h4
						class="text-xs font-bold uppercase tracking-wider text-slate-400 font-mono">
						Daily Summary (오늘 하루 총평)</h4>
					<p
						class="text-base text-slate-800 leading-relaxed font-sans bg-slate-50/60 p-4 rounded-xl border border-slate-200 whitespace-pre-line">
				...
				<div class="pt-2">
					<c:choose>
						...
						<c:when test="${not empty review.mainImageUrl}">
							<div
								class="w-full border-2 border-dashed border-slate-300 rounded-xl overflow-hidden bg-slate-50 relative group">
								<img src="${review.mainImageUrl}" alt="오늘 하루 대표 이미지"
									class="w-full h-auto block rounded-lg transition-transform group-hover:scale-[1.005]" />
								<div
									class="absolute bottom-3 right-3 bg-slate-900/80 backdrop-blur-xs text-white text-[11px] font-mono px-2.5 py-1 rounded-md shadow-xs">
						...
						<c:otherwise>
							<div
								class="w-full h-56 border-2 border-dashed border-slate-300 rounded-lg bg-slate-100/70 flex flex-col items-center justify-center p-4 text-center text-slate-500 hover:bg-slate-100 transition-colors">
								<div
									class="w-10 h-10 rounded-full bg-slate-200/80 flex items-center justify-center mb-2 text-slate-400">
									<i class="fa-regular fa-image text-lg"></i>
								</div>
								<span class="font-mono text-xs font-semibold text-slate-600">
									[오늘 하루 대표 이미지 영역] </span> <span
									class="text-[11px] text-slate-400 mt-1 max-w-xs"> 하루의 대표
									하이라이트 사진 / 영수증 인증 샷 </span>
							</div>
						</c:otherwise>
					</c:choose>
				</div>
			...
			<section class="space-y-4">
				<div
					class="flex items-center justify-between pb-2 border-b-2 border-dashed border-slate-300">
					<div class="flex items-center gap-2">
				...
				<c:choose>
					<c:when test="${not empty subReviews}">
						<div class="space-y-4">
							<c:forEach items="${subReviews}" var="sub">
								<div
									class="border-2 border-dashed border-slate-300 rounded-lg p-4 bg-white/90 shadow-sm space-y-3 relative group transition-all hover:border-slate-400">
									...
									<div class="flex items-center justify-between">
										<div class="flex items-center gap-2">
											...
											<c:choose>
												<c:when test="${sub.category eq 'place'}">
													<span
														class="inline-flex items-center gap-1 font-medium border border-dashed rounded-md px-2.5 py-1 text-xs bg-emerald-50 text-emerald-700 border-emerald-300">
														<i class="fa-solid fa-mug-hot text-xs"></i> <span>장소·식당·카페</span>
													</span>
												</c:when>
												<c:when test="${sub.category eq 'item'}">
													<span
														class="inline-flex items-center gap-1 font-medium border border-dashed rounded-md px-2.5 py-1 text-xs bg-sky-50 text-sky-700 border-sky-300">
														<i class="fa-solid fa-laptop text-xs"></i> <span>아이템·전자기기</span>
													</span>
												</c:when>
												<c:when test="${sub.category eq 'transport'}">
													<span
														class="inline-flex items-center gap-1 font-medium border border-dashed rounded-md px-2.5 py-1 text-xs bg-amber-50 text-amber-700 border-amber-300">
														<i class="fa-solid fa-car text-xs"></i> <span>차량·이동수단</span>
													</span>
												</c:when>
												<c:when test="${sub.category eq 'content'}">
													<span
														class="inline-flex items-center gap-1 font-medium border border-dashed rounded-md px-2.5 py-1 text-xs bg-purple-50 text-purple-700 border-purple-300">
														<i class="fa-solid fa-clapperboard text-xs"></i> <span>미디어·콘텐츠</span>
													</span>
												</c:when>
												<c:otherwise>
													<span
														class="inline-flex items-center gap-1 font-medium border border-dashed rounded-md px-2.5 py-1 text-xs bg-slate-100 text-slate-700 border-slate-300">
														<i class="fa-solid fa-tag text-xs"></i> <span>${sub.category}</span>
													</span>
												</c:otherwise>
											</c:choose>
											...
											<c:if test="${sub.isCertified eq 'Y'}">
												<span
													class="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">
													<i
													class="fa-regular fa-circle-check text-emerald-600 text-xs"></i>
													<span>내돈내산 인증</span>
												</span>
											</c:if>
										</div>
										...
										<div class="inline-flex items-center gap-1.5 select-none">
												...
												<c:forEach var="si" begin="1" end="5">
													<c:choose>
														...
														<c:when test="${sub.subRating >= (si - 0.5)}">
															<i
																class="fa-solid fa-star-half-stroke text-amber-400 text-xs"></i>
														</c:when>
														<c:otherwise>
											...
											<span class="text-slate-700 ml-1 font-mono text-xs">
												${sub.subRating} <span class="text-slate-400 font-normal">/
													5.0</span>
											</span>
										</div>
									</div>
									...
									<div>
										<h4
											class="text-base font-bold text-slate-800 flex items-center gap-1.5">
									...
									<!-- Comment / Review text -->
									<p
										class="text-sm text-slate-700 bg-slate-50 p-2.5 rounded border border-slate-200 leading-relaxed font-sans whitespace-pre-line">
										...
										<div class="flex flex-wrap gap-1.5 pt-1">
											<c:forEach items="${fn:split(sub.tags, ',')}" var="subTag">
												<c:if test="${not empty fn:trim(subTag)}">
													<span
														class="inline-flex items-center gap-1 bg-slate-100 text-slate-700 border border-slate-300 rounded font-mono px-1.5 py-0.5 text-xs">
														<i class="fa-solid fa-tag text-[10px] text-slate-400"></i>
														<span><c:out
																value="${fn:startsWith(fn:trim(subTag), '#') ? fn:trim(subTag) : '#' += fn:trim(subTag)}" /></span>
													</span>
												</c:if>
											</c:forEach>
										</div>
					...
					<c:otherwise>
						<div
							class="p-8 border-2 border-dashed border-slate-300 rounded-lg bg-white/90 text-center space-y-2">
							<p class="text-xs text-slate-500 font-mono">등록된 세부 서브 리뷰가
								없습니다.</p>
						</div>
					</c:otherwise>
				</c:choose>
			...
			<c:if test="${not empty allSponsored}">
				<section
					class="border-2 border-dashed border-amber-300 rounded-2xl bg-amber-50/40 p-6 space-y-4">
					<div
						class="flex items-center justify-between pb-3 border-b border-amber-200">
						<div class="flex items-center gap-2">
							<i class="fa-solid fa-wand-magic-sparkles text-amber-600 text-lg"></i>
							<h3 class="text-base font-bold text-amber-950">BM 제휴 큐레이션: 이
								날의 추천 아이템 & 장소 최저가</h3>
						</div>
						<span
							class="text-xs font-mono text-amber-700 bg-amber-200/60 px-2 py-0.5 rounded font-semibold">
							제휴 파트너스 연동 </span>
					</div>

					<p class="text-xs text-amber-800 leading-relaxed">리뷰어가 직접 경험하고
						높은 평점을 남긴 장소/아이템의 실시간 최저가 및 예약 링크입니다. 구매 및 예약 시 플랫폼과 리뷰어에게 소정의
						리워드가 제공됩니다.</p>

					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
						<c:forEach items="${allSponsored}" var="sp">
							<div
								class="border-2 border-dashed border-amber-300 bg-amber-50/50 rounded-xl p-3.5 space-y-3 transition-all hover:border-amber-400 w-full">
								<div
									class="flex items-center justify-between gap-1 pb-2 border-b border-amber-200/80">
									<div class="flex items-center gap-1.5 min-w-0 flex-1">
										<span
											class="bg-amber-400 text-amber-950 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded shrink-0">
											BM 제휴 광고 </span> <span
											class="text-xs text-amber-900 font-medium truncate flex items-center gap-1">
											<i class="fa-solid fa-sparkles text-amber-600 text-xs"></i> <span
											class="truncate">${sp.category} 연계 제휴</span>
										</span>
									</div>
									<span
										class="text-[10px] text-amber-700 font-mono shrink-0 hidden sm:inline">
										파트너스 추천 </span>
								</div>

								<div class="flex items-start gap-2.5">
									<div
										class="w-10 h-10 rounded-lg bg-amber-100 border border-dashed border-amber-300 flex items-center justify-center text-amber-700 shrink-0">
										<i class="fa-solid fa-bag-shopping text-base"></i>
									</div>
									<div class="min-w-0 flex-1">
										<div class="flex flex-wrap items-center gap-1.5">
											<span
												class="text-[10px] font-bold text-amber-900 bg-amber-200/80 px-1.5 py-0.5 rounded">
												${sp.merchantName} </span>
										</div>
								...
								<div class="pt-0.5">
									<a href="${empty sp.affiliateUrl ? '#' : sp.affiliateUrl}"
										...
										<span>제휴 최저가 확인</span> <i
										class="fa-solid fa-arrow-up-right-from-square text-[11px]"></i>
									</a>
									<p
										class="text-[10px] text-slate-400 text-center mt-1 font-mono">
										* 구매/예약 시 소정의 파트너스 수수료가 정산됩니다</p>
								</div>
							</div>
						</c:forEach>
					</div>
			...
			<!-- --------------------------------------------------------------------- -->
			<section
				class="border-2 border-dashed border-slate-300 rounded-2xl bg-white p-6 space-y-4">
				<div
				...
				<c:choose>
					...
					<c:when test="${not empty sessionScope.loginUser}">
						<form id="commentForm"
							...
							<div
								class="flex justify-between items-center text-[10px] text-slate-400 font-mono px-1">
								<span>* 부적절한 언어 사용 시 삭제될 수 있습니다.</span> <span
									id="commentLengthCounter">0 / 500자</span>
							</div>
						</form>
					</c:when>
			...
			<!-- --------------------------------------------------------------------- -->
			<div
				class="border-2 border-dashed border-blue-300 rounded-xl bg-blue-50/60 p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
